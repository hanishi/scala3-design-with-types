...
Java のジェネリクスはデフォルトで不変です。`List<Book>` を `List<Item>` として
受け取るには、**呼び出し側で毎回** `? extends` を書く必要があります：
...
これは*使用側*変位です ― `? extends` を書き忘れると、サブタイプを黙って
受け付けなくなります。柔軟にしたいメソッドすべてにアノテーションが必要で、
書き忘れても何も警告されません。

Scala の `List[+A]` はクラス定義で共変を一度だけ宣言します ―
`cheapest(books)` はどこでもそのまま動き、忘れようがありません。
...
明示的に `Predicate<? super T>` を受け取るからです ― 使用側反変：
...
`Predicate<Item>` を `Predicate<Book>` が期待される場所に渡すことはできません ―
しかもアノテーションが抜けていても何も警告されません。

Scala の `Function1[-A, +B]` は反変を一度だけ宣言します ―
`books.filter(cheap)` はそのまま動き、呼び出し側は何も意識しません。
...
反変の脱出口は問題になりません ― `String` や `Boolean` を返し、`A` は返さない
からです。しかし反変型を合成するときには問題になります ― 実際の例で見るのが
